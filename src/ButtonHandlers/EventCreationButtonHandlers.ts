...
const eventCreationButtonHandlers: {
    ...
    addImage: async (
        ...
        await submissionInteraction.editReply({
            content:
                "Adding image. Please be patient! If your image is large, this may take a minute.",
            embeds: [],
            components: [],
        });
    ...
    finish: async (
        ...
        if (
            ...
            const member =
                await submissionInteraction.message.guild.members.fetch(
                    event.author.id
                );
            if (
